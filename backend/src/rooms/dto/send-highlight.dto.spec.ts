...
import { SendHighlightDto } from './send-highlight.dto';

const validCoord = { pageIndex: 0, left: 10, top: 20, width: 30, height: 5 };

function make(overrides: object = {}): SendHighlightDto {
  return plainToInstance(SendHighlightDto, {
    roomId: 'room-abc',
    page: 0,
    coords: [validCoord],
    ...overrides,
...
async function errors(dto: SendHighlightDto) {
  return collectErrors(await validate(dto));
}

describe('SendHighlightDto', () => {
  ...
  it('fails when a coord has a negative left value', async () => {
    const e = await errors(make({ coords: [{ ...validCoord, left: -1 }] }));
    expect(e.length).toBeGreaterThan(0);
  });

  it('fails when a coord has a negative top value', async () => {
    const e = await errors(make({ coords: [{ ...validCoord, top: -5 }] }));
    expect(e.length).toBeGreaterThan(0);
  });
  ...
  it('fails when a coord has height over 100', async () => {
    const e = await errors(
      make({ coords: [{ ...validCoord, height: 100.1 }] }),
    );
    expect(e.length).toBeGreaterThan(0);
  });

  it('fails when content exceeds 1000 characters', async () => {
    const e = await errors(make({ content: 'x'.repeat(1001) }));
    expect(e.length).toBeGreaterThan(0);
  });

  it('passes when content is exactly 1000 characters', async () => {
    expect(await errors(make({ content: 'x'.repeat(1000) }))).toHaveLength(0);
  });

  it('passes when content is omitted (optional)', async () => {
    expect(await errors(make({ content: undefined }))).toHaveLength(0);
  });
});
